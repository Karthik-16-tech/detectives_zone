...
export function WhatsAppFloatingButton({
  ...
  const [activeMessage, setActiveMessage] = useState<string>(
    message || "Hi Detectives Zone Team, I have an inquiry."
  );
      ...
      api
        ...
        .then((s) => {
          if (s) {
            ...
            } else if (s.whatsapp_position === "bottom-left" || s.whatsapp_position === "bottom-right") {
  ...
  return (
    <a
      ...
      aria-label="Encrypted Dispatch Support on WhatsApp"
      title={`Dispatch Support via WhatsApp (${activeNumber})`}
      className={`group fixed z-[999] flex h-11 w-11 sm:h-12 sm:w-12 items-center justify-center rounded-full bg-black hover:bg-[#121212] border border-[#25D366]/40 hover:border-[#25D366] text-[#25D366] shadow-[0_4px_16px_rgba(0,0,0,0.9)] transition-all duration-300 hover:scale-105 active:scale-95 ${posClasses} ${className}`}
      style={{
        textDecoration: "none",
        boxShadow: "0 4px 16px rgba(0, 0, 0, 0.9)",
      }}
    >
      {/* Official WhatsApp Logo in Green */}
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 32 32"
        className="h-6 w-6 fill-[#25D366] group-hover:scale-110 transition-transform duration-300"
      >
