...
function AdminContactCMS() {
  const [activeTab, setActiveTab] = useState<"channels" | "form" | "inbox">("channels");
  ...
  // Section 2: "Confidential report: Open a new case"
  ...
  const loadInboxData = async (status?: string) => {
    try {
  ...
  const handleUpdateStatus = async (id: number, status: string) => {
    try {
      ...
      setInbox(inbox.map((m) => (m.id === id ? { ...m, status } : m)));
      ...
      action={
        <div className="flex items-center gap-2">
          <button
            ...
            className={`rounded-lg px-3.5 py-2 font-display text-[11px] font-semibold uppercase tracking-wider transition-all cursor-pointer ${
              activeTab === "channels"
                ? "bg-blood text-white shadow-[0_0_15px_rgba(179,18,23,0.4)]"
          ...
          >
            Confidential Report Block
          </button>
          <button
            onClick={() => setActiveTab("inbox")}
            className={`flex items-center gap-1.5 rounded-lg px-3.5 py-2 font-display text-[11px] font-semibold uppercase tracking-wider transition-all cursor-pointer ${
              activeTab === "inbox"
                ? "bg-blood text-white shadow-[0_0_15px_rgba(179,18,23,0.4)]"
                : "border border-white/10 bg-white/[0.03] text-white/60 hover:text-white"
            }`}
          >
            ...
            <span>Inquiries ({inbox.length})</span>
          </button>
        </div>
      }
    >
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          TAB 1: FOUR WAYS TO REACH THE DESK
      ═══════════════════════════════════════════════════════════════════ */}
      {activeTab === "channels" && (
        <form onSubmit={handleSaveChannels} className="space-y-8 font-mono text-xs">
          {/* Header Metric Editor */}
          <div className="rounded-2xl border border-white/10 bg-[#080808] p-6 sm:p-7">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div>
                <label className="block uppercase text-white/60 mb-1.5">Section Kicker</label>
                <input
                  type="text"
                  required
                  value={channels.section_kicker}
                  onChange={(e) => setChannels({ ...channels, section_kicker: e.target.value })}
              ...
              <div>
                <label className="block uppercase text-white/60 mb-1.5">Title Text</label>
                <input
                  type="text"
                  required
                  value={channels.section_title}
                  onChange={(e) => setChannels({ ...channels, section_title: e.target.value })}
                  placeholder="Four ways to reach"
                  className="w-full rounded-lg border border-white/10 bg-black/60 p-2.5 text-white outline-none focus:border-blood"
                />
              </div>
              <div>
                <label className="block uppercase text-white/60 mb-1.5">Accent (Red Highlight)</label>
                <input
                  type="text"
                  required
                  value={channels.section_accent}
                  onChange={(e) => setChannels({ ...channels, section_accent: e.target.value })}
            ...
            <div className="rounded-2xl border border-white/10 bg-[#080808] p-6 space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-white/10">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-display text-sm font-bold uppercase text-white">Channel 1</span>
                </div>
                <input
                  type="text"
                  value={channels.ch1_file}
                  onChange={(e) => setChannels({ ...channels, ch1_file: e.target.value })}
                  className="rounded border border-white/10 bg-black/60 px-2 py-1 text-[10px] text-white/70 text-right w-28"
                />
              </div>
              <div>
                <label className="block uppercase text-white/60 mb-1">Title</label>
                <input
                  type="text"
                  value={channels.ch1_title}
                  onChange={(e) => setChannels({ ...channels, ch1_title: e.target.value })}
                  className="w-full rounded-lg border border-white/10 bg-black/60 p-2 text-white outline-none focus:border-blood"
                />
              </div>
              <div>
                <label className="block uppercase text-white/60 mb-1">Email / Line 1</label>
                <input
                  type="text"
                  value={channels.ch1_line1}
                  onChange={(e) => setChannels({ ...channels, ch1_line1: e.target.value })}
                  className="w-full rounded-lg border border-white/10 bg-black/60 p-2 text-white outline-none focus:border-blood"
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block uppercase text-white/60 mb-1">Response Time / Line 2</label>
                  <input
                    type="text"
                    value={channels.ch1_line2}
                    onChange={(e) => setChannels({ ...channels, ch1_line2: e.target.value })}
                    className="w-full rounded-lg border border-white/10 bg-black/60 p-2 text-white outline-none focus:border-blood"
                  />
                </div>
                <div>
                  <label className="block uppercase text-white/60 mb-1">PGP Key / Line 3</label>
                  <input
                    type="text"
                    value={channels.ch1_line3}
                    onChange={(e) => setChannels({ ...channels, ch1_line3: e.target.value })}
                    className="w-full rounded-lg border border-white/10 bg-black/60 p-2 text-white outline-none focus:border-blood"
                  />
                </div>
              </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-[#080808] p-6 space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-white/10">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-display text-sm font-bold uppercase text-white">Channel 2</span>
                </div>
                <input
                  type="text"
                  value={channels.ch2_file}
                  onChange={(e) => setChannels({ ...channels, ch2_file: e.target.value })}
                  className="rounded border border-white/10 bg-black/60 px-2 py-1 text-[10px] text-white/70 text-right w-28"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setChannels({ ...channels, ch2_title: e.target.value })}
                  className="w-full rounded-lg border border-white/10 bg-black/60 p-2 text-white outline-none focus:border-blood"
                />
              </div>
              <div>
                <label className="block uppercase text-white/60 mb-1">Phone Number / Line 1</label>
                <input
                  type="text"
                  value={channels.ch2_line1}
                  onChange={(e) => setChannels({ ...channels, ch2_line1: e.target.value })}
                  className="w-full rounded-lg border border-white/10 bg-black/60 p-2 text-white outline-none focus:border-blood"
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block uppercase text-white/60 mb-1">Operating Hours / Line 2</label>
                  <input
                    type="text"
                    value={channels.ch2_line2}
                    onChange={(e) => setChannels({ ...channels, ch2_line2: e.target.value })}
                    className="w-full rounded-lg border border-white/10 bg-black/60 p-2 text-white outline-none focus:border-blood"
                  />
                </div>
                <div>
                  <label className="block uppercase text-white/60 mb-1">Security Note / Line 3</label>
                  <input
                    type="text"
                    value={channels.ch2_line3}
                    onChange={(e) => setChannels({ ...channels, ch2_line3: e.target.value })}
                    className="w-full rounded-lg border border-white/10 bg-black/60 p-2 text-white outline-none focus:border-blood"
                  />
                </div>
              </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-[#080808] p-6 space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-white/10">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-display text-sm font-bold uppercase text-white">Channel 3</span>
                </div>
                <input
                  type="text"
                  value={channels.ch3_file}
                  onChange={(e) => setChannels({ ...channels, ch3_file: e.target.value })}
                  className="rounded border border-white/10 bg-black/60 px-2 py-1 text-[10px] text-white/70 text-right w-28"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setChannels({ ...channels, ch3_title: e.target.value })}
                  className="w-full rounded-lg border border-white/10 bg-black/60 p-2 text-white outline-none focus:border-blood"
                />
              </div>
              <div>
                <label className="block uppercase text-white/60 mb-1">Address / Line 1</label>
                <input
                  type="text"
                  value={channels.ch3_line1}
                  onChange={(e) => setChannels({ ...channels, ch3_line1: e.target.value })}
                  className="w-full rounded-lg border border-white/10 bg-black/60 p-2 text-white outline-none focus:border-blood"
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block uppercase text-white/60 mb-1">Suite / City / Line 2</label>
                  <input
                    type="text"
                    value={channels.ch3_line2}
                    onChange={(e) => setChannels({ ...channels, ch3_line2: e.target.value })}
                    className="w-full rounded-lg border border-white/10 bg-black/60 p-2 text-white outline-none focus:border-blood"
                  />
                </div>
                <div>
                  <label className="block uppercase text-white/60 mb-1">Access Policy / Line 3</label>
                  <input
                    type="text"
                    value={channels.ch3_line3}
                    onChange={(e) => setChannels({ ...channels, ch3_line3: e.target.value })}
                    className="w-full rounded-lg border border-white/10 bg-black/60 p-2 text-white outline-none focus:border-blood"
                  />
                </div>
              </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-[#080808] p-6 space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-white/10">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-display text-sm font-bold uppercase text-white">Channel 4</span>
                </div>
                <input
                  type="text"
                  value={channels.ch4_file}
                  onChange={(e) => setChannels({ ...channels, ch4_file: e.target.value })}
                  className="rounded border border-white/10 bg-black/60 px-2 py-1 text-[10px] text-white/70 text-right w-28"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setChannels({ ...channels, ch4_title: e.target.value })}
                  className="w-full rounded-lg border border-white/10 bg-black/60 p-2 text-white outline-none focus:border-blood"
                />
              </div>
              <div>
                <label className="block uppercase text-white/60 mb-1">Priority Email / Line 1</label>
                <input
                  type="text"
                  value={channels.ch4_line1}
                  onChange={(e) => setChannels({ ...channels, ch4_line1: e.target.value })}
                  className="w-full rounded-lg border border-white/10 bg-black/60 p-2 text-white outline-none focus:border-blood"
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block uppercase text-white/60 mb-1">Availability / Line 2</label>
                  <input
                    type="text"
                    value={channels.ch4_line2}
                    onChange={(e) => setChannels({ ...channels, ch4_line2: e.target.value })}
                    className="w-full rounded-lg border border-white/10 bg-black/60 p-2 text-white outline-none focus:border-blood"
                  />
                </div>
                <div>
                  <label className="block uppercase text-white/60 mb-1">Routing Note / Line 3</label>
                  <input
                    type="text"
                    value={channels.ch4_line3}
                    onChange={(e) => setChannels({ ...channels, ch4_line3: e.target.value })}
                    className="w-full rounded-lg border border-white/10 bg-black/60 p-2 text-white outline-none focus:border-blood"
                  />
                </div>
              </div>
          ...
          <div className="flex justify-end pt-2">
            ...
            >
              ...
              <span>{saving ? "Saving..." : "Save Four Channels Live"}</span>
            </button>
          </div>
        </form>
      )}
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          TAB 2: CONFIDENTIAL REPORT: OPEN A NEW CASE BLOCK
      ═══════════════════════════════════════════════════════════════════ */}
      {activeTab === "form" && (
        <form onSubmit={handleSaveForm} className="space-y-6 font-mono text-xs max-w-3xl">
          <div className="rounded-2xl border border-blood/30 bg-gradient-to-b from-blood/[0.08] to-transparent p-6 sm:p-8 space-y-5">
            <div className="flex items-center gap-3 pb-4 border-b border-white/10">
              <ShieldCheck className="h-6 w-6 text-blood" />
              <div>
                <h3 className="font-display text-base font-bold uppercase tracking-wider text-white">
                  Confidential Report Section Header & Description
                </h3>
                <p className="font-mono text-[10px] text-white/50 uppercase tracking-wider mt-0.5">
                  Direct Case Intake Presentation & Encryption Notice
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormSettings({ ...formSettings, kicker: e.target.value })}
              ...
              <div>
                <label className="block uppercase text-white/60 mb-1.5">Accent Word</label>
                <input
                  type="text"
                  required
                  value={formSettings.accent}
                  onChange={(e) => setFormSettings({ ...formSettings, accent: e.target.value })}
                  placeholder="case"
                  className="w-full rounded-lg border border-white/10 bg-black/60 p-2.5 text-white outline-none focus:border-blood"
                />
              </div>
            ...
            <div>
              <label className="block uppercase text-white/60 mb-1.5">
                Encryption & Privacy Notice (Description)
              </label>
              <textarea
                rows={3}
                required
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-2 border-t border-white/10">
              <div>
                <label className="block uppercase text-white/60 mb-1.5">Support Direct Line</label>
                <input
                  type="text"
                  value={formSettings.support_phone}
                  onChange={(e) => setFormSettings({ ...formSettings, support_phone: e.target.value })}
                  placeholder="+91 63057 29867"
                  className="w-full rounded-lg border border-white/10 bg-black/60 p-2.5 text-white outline-none focus:border-blood"
                />
              </div>
              <div>
                <label className="block uppercase text-white/60 mb-1.5">Support Availability Status</label>
                <input
                  type="text"
                  value={formSettings.support_hours}
                  onChange={(e) => setFormSettings({ ...formSettings, support_hours: e.target.value })}
          ...
          <div className="flex justify-end pt-2">
            ...
            >
              <Save className="h-4 w-4" />
              <span>{saving ? "Saving..." : "Save Confidential Report Block Live"}</span>
            </button>
          </div>
        </form>
      )}
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          TAB 3: INQUIRIES & CASE REPORTS INBOX
      ═══════════════════════════════════════════════════════════════════ */}
      {activeTab === "inbox" && (
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              {["ALL", "unread", "in_progress", "resolved"].map((s) => (
                <button
                  key={s}
                  onClick={() => {
                    setInboxFilter(s);
                  ...
                  className={`rounded-lg px-3 py-1.5 font-mono text-[11px] uppercase transition-all cursor-pointer ${
                    inboxFilter === s
          ...
          {inbox.length === 0 ? (
            <div className="rounded-2xl border border-white/10 bg-[#080808] p-12 text-center font-mono text-xs text-white/40">
              <Mail className="mx-auto h-8 w-8 text-white/20 mb-3" />
              <p>No inquiries found under filter: {inboxFilter}</p>
            </div>
          ) : (
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-white/[0.08]">
                    <div className="flex items-center gap-3">
                      <span className="font-display text-sm font-bold uppercase text-white">{msg.name}</span>
                      <span className="text-white/40">({msg.email})</span>
                      {msg.phone && <span className="text-white/40">📱 {msg.phone}</span>}
                    </div>
                    <div className="flex items-center gap-2">
                      <select
                        value={msg.status}
                  ...
                  <div className="mt-3">
                    <div className="font-bold text-white mb-1">Subject: {msg.subject}</div>
                    <p className="text-white/70 leading-relaxed bg-black/40 p-3 rounded-lg border border-white/5 whitespace-pre-wrap">
                      {msg.message}
                    </p>
                  </div>

                  <div className="mt-3 flex items-center justify-between text-[10px] text-white/40 pt-2 border-t border-white/[0.04]">
                    <span>Case Interest: {msg.case_interest || "General Inquiry"}</span>
